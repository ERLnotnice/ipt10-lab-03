...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const complete_name = document.querySelector('input[name="complete_name"]');
            const email = document.querySelector('input[name="email"]');
            const submitButton = document.querySelector('button[type="submit"]');

            function validateForm() {
                const nameValid = complete_name.value.trim() !== '';
                const emailValid = email.value.trim() !== '' && email.checkValidity();

                submitButton.disabled = !(nameValid && emailValid);
            }

            complete_name.addEventListener('input', validateForm);
